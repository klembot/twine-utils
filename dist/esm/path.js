...
export function storyDirectorySync() {
    ...
    const docDirectory = documentDirectoryNames.find(dir => testDir(path.join(result, dir)));
    if (!docDirectory) {
        throw new Error('Could not find your documents directory');
    }
    result = path.join(result, docDirectory);
    // Find the Twine directory.
    const twineDirectory = twineDirectoryNames.find(dir => testDir(path.join(result, dir)));
    if (!twineDirectory) {
    ...
    const storiesDirectory = storyDirectoryNames.find(dir => testDir(path.join(result, dir)));
    if (!storiesDirectory) {
